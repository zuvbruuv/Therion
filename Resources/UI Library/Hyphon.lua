...
function library:create_settings_tab(menu)
    ...
    task.spawn(function()
        ...
        if isfile(autoLoad) then
            local configToLoad = readfile(autoLoad)
            if configToLoad and #configToLoad > 0 then
                library:load_config(configToLoad)
                library:notification(("Auto-loaded config '%s'"):format(configToLoad), 5, color3_new(0.35, 1, 0.35))
            end
        end
    end)
