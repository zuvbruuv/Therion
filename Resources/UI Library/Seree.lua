...
--]]
local drawing = loadstring(game:HttpGet("https://gist.githubusercontent.com/0f76/9dc85c8c380d895373dd306fd372fa59/raw/e2abc40c2b5f159d61b10558c86e4f98823e30f5/drawing_extension.lua"))()
local tween = loadstring(game:HttpGet("https://raw.githubusercontent.com/zuvbruuv/Therion/refs/heads/main/Libraries/Tween.lua"))();
...
local themeobjects = {};
local library = {notiflist = {ntifs = {}, interval = 12}, settings = {folder_name = "therion";default_accent = Color3.fromRGB(255,255,255)}, drawings = {}, theme = table.clone(themes.Default),currentcolor = nil, flags = {}, open = false, mousestate = services.InputService.MouseIconEnabled, cursor = nil, holder = nil, connections = {}, notifications = {}, gradient = nil};
...
--
local images = {
    ['gradient'] = "https://raw.githubusercontent.com/zuvbruuv/Therion/refs/heads/main/Resources/Images/gradient_image.png";
}
for i,v in next, images do
    if not isfile(library.settings.folder_name..'/assets/'..i..'.ln') then
        writefile(library.settings.folder_name..'/assets/'..i..'.ln', crypt.encrypt('aes-ctr',game:HttpGet(v),'4XGudgFuutoHUM2Ctwsq4YrQ','zP5JJWPSIbf5Xuuy'))
    end
    images[i] = crypt.decrypt('aes-ctr',readfile(library.settings.folder_name..'/assets/'..i..'.ln'),'4XGudgFuutoHUM2Ctwsq4YrQ','zP5JJWPSIbf5Xuuy')
end
-- // Functions
...
function library.object_colorpicker(default, defaultalpha, parent, count, flag, callback, offset)
    ...
    local animation_rainbow = library:create("Text", {
        ...
        Font = Drawing.Fonts.Plex,
        Size = 13,
        Position = UDim2.new(0.5, -utility.textlength("rainbow", 2, 13).X - 2, 0.5, -60),
        Center = true,
        Theme = "Text",
        ZIndex = 26,
        Outline = false,
    ...
    local rainbowslider = library.createslider({parent = rainbow_page, name = "speed", flag = library.flags[flag .. "_RAINBOW_SPEED"], min = 0, max = 100, default = 10, callback = function(state) library.flags[flag .. "_RAINBOW_SPEED"] = state end})
    local fadeslider = library.createslider({parent = fade_page, name = "speed", flag = library.flags[flag .. "_FADE_SPEED"], min = 0, max = 100, default = 10, callback = function(state) library.flags[flag .. "_FADE_SPEED"] = state end})

    local fadestart = library.createpicker({parent = fade_page, name = "start color", flag = library.flags[flag .. "_FADE_START"], default = Color3.new(1,1,1), callback = function(state) library.flags[flag .. "_FADE_START"] = state end})
    local fadeend = library.createpicker({parent = fade_page, name = "end color", flag = library.flags[flag .. "_FADE_END"], default = Color3.new(0,0,0), callback = function(state) library.flags[flag .. "_FADE_END"] = state end})

    local rainbow_button = library:create("Square", {Parent = animationpage, Size = UDim2.new(0, utility.textlength("rainbow",2,13).X,0,utility.textlength("rainbow",2,13).Y +2), Position = UDim2.new(0.5, -utility.textlength("rainbow", 2, 13).X - 26, 0.5, -62), Color = Color3.fromRGB(0,0,0), Thickness = 1, Transparency = 0, Filled = false, ZIndex = 21});
    local fade_button = library:create("Square", {Parent = animationpage, Size = UDim2.new(0, utility.textlength("fading",2,13).X,0,utility.textlength("fading",2,13).Y +2), Position = UDim2.new(0.5, utility.textlength("fading", 2, 13).X - 12, 0.5, -62), Color = Color3.fromRGB(0,0,0), Thickness = 1, Transparency = 0, Filled = false, ZIndex = 21});
    ...
    task.spawn(function()
        while wait() do
            if library.flags[flag .. "_RAINBOW"] then
                ...
                set(Color, library.flags[flag], false, true)
            end
            if library.flags[flag .. "_FADE"] then
                local progress = (math.sin(2 * math.pi * (library.flags[flag .. "_FADE_SPEED"] / 10) * time) + 1) / 2;
                local value = library.flags[flag .. "_FADE_START"]:Lerp(library.flags[flag .. "_FADE_END"], progress)

                set(value, library.flags[flag], false, true);
...
function library:new(cfg)
    -- // Configuration
    local window = {pages = {}, pages_buttons = {}, pages_titles = {}, pages_buttons_lines = {}};
    ...
    local size = cfg.size or cfg.Size or Vector2.new(600, 525);
    -- // Drawings
    local window_outline = library:create("Square", {Visible = false, Transparency = 1, Color = Color3.fromRGB(45,45,45), Size = UDim2.new(0,size.X,0,size.Y), Position = UDim2.new(0.5, -(size.X / 2), 0.5, -(size.Y / 2)), Thickness = 1, Filled = true, ZIndex = 10}) do
        local outline = library:outline(window_outline, Color3.fromRGB(62,62,62), 10);
        library:outline(outline, Color3.new(0,0,0), 10);
    end;
    --
    library.holder = window_outline;
    --
    local window_inline = library:create("Square", {Parent = window_outline, Visible = true, Transparency = 1, Color = Color3.fromRGB(12,12,12), Size = UDim2.new(1,-10,1,-10), Position = UDim2.new(0,5,0,5), Thickness = 1, Filled = true, ZIndex = 11}) do
        local outline = library:outline(window_inline, Color3.fromRGB(54,54,54), 11);
    end;
    --
    local window_accent = library:create("Square", {Parent = window_inline, Visible = true, Transparency = 1, Theme = "Accent", Size = UDim2.new(1,-2,0,2), Position = UDim2.new(0,1,0,1), Thickness = 1, Filled = true, ZIndex = 11});
    --
    local window_holder = library:create("Square", {Parent = window_inline, Visible = true, Transparency = 1, Color = Color3.fromRGB(13,13,13), Size = UDim2.new(1,-30,1,-85), Position = UDim2.new(0,15,0,70), Thickness = 1, Filled = true, ZIndex = 12}) do
        local outline = library:outline(window_holder, Color3.fromRGB(32,32,32), 12);
        library:outline(outline, Color3.fromRGB(0,0,0), 12);
    end;
    --
    local window_title_holder = library:create("Square", {Parent = window_inline, Visible = true, Transparency = 1, Color = Color3.fromRGB(13,13,13), Size = UDim2.new(0,100,0,40), Position = UDim2.new(0,15,0,20), Thickness = 1, Filled = true, ZIndex = 12}) do
        local outline = library:outline(window_title_holder, Color3.fromRGB(32,32,32), 12);
        library:outline(outline, Color3.fromRGB(0,0,0), 12);
    end;
    --
    local window_page_holder = library:create("Square", {Parent = window_inline, Visible = true, Transparency = 1, Color = Color3.fromRGB(13,13,13), Size = UDim2.new(1,-150,0,40), Position = UDim2.new(0,135,0,20), Thickness = 1, Filled = true, ZIndex = 12}) do
        local outline = library:outline(window_page_holder, Color3.fromRGB(32,32,32), 12);
        library:outline(outline, Color3.fromRGB(0,0,0), 12);
    end;
    --
    local window_page_holder_inline = library:create("Square", {Parent = window_page_holder, Visible = true, Transparency = 0, Size = UDim2.new(1,-8,1,-4), Position = UDim2.new(0,4,0,2), Thickness = 1, Filled = true, ZIndex = 12})
    --
    local window_drag = library:create("Square", {Parent = window_outline, Visible = true, Transparency = 0, Size = UDim2.new(1,0,0,10), Position = UDim2.new(0,0,0,0), Thickness = 1, Filled = true, ZIndex = 50})
    --
    ...
    --
    local window_title = library:create("Text", {Text = name_white, Parent = window_title_holder, Visible = true, Transparency = 1, Theme = "Text", Size = 13, Center = true, Outline = true, Font = Drawing.Fonts.Plex, Position = UDim2.new(0.5,-utility.textlength(name_white,2,13).X/2 + offset,0,14), ZIndex = 13});
    local window_title_accent = library:create("Text", {Text = name_color, Parent = window_title_holder, Visible = true, Transparency = 1, Theme = "Accent", Size = 13, Center = true, Outline = true, Font = Drawing.Fonts.Plex, Position = UDim2.new(0.5,utility.textlength(name_color,2,13).X/2 + offset,0,14), ZIndex = 13});
    --
    ...
    function window:page(cfg)
        ...
        -- // Drawings
        local button_holder = library:create("Square", {Parent = window_page_holder_inline, Visible = true, Transparency = 0, Thickness = 1, Filled = true, ZIndex = 14});
        table.insert(self.pages_buttons, button_holder);
        --
        local button_accent = library:create("Square", {Parent = button_holder, Visible = true, Transparency = 1, Theme = "Un-Selected", Size = UDim2.new(1,-6,0,2), Position = UDim2.new(0,3,1,-2), Thickness = 1, Filled = true, ZIndex = 13}); do
            library:outline(button_accent, Color3.new(0,0,0), 13);
            local gradient = library:create("Image", {Data = images.gradient, Transparency = 1, Visible = true, Parent = button_accent, Size = UDim2.new(1,0,1,0), ZIndex = 14});
            table.insert(self.pages_buttons_lines, button_accent);
        end;
        --
        local button_title = library:create("Text", {Text = name, Parent = button_holder, Visible = true, Transparency = 1, Theme = "Un-Selected_Text", Size = 13, Center = true, Outline = true, Font = Drawing.Fonts.Plex, Position = UDim2.new(0.5,0,0,8), ZIndex = 13});
        ...
        for _,v in next, self.pages_buttons do
            v.Size = UDim2.new(1 / #self.pages_buttons, _ == 1 and 1 or _ == #self.pages_buttons and -2 or -1, 1, -4);
            v.Position = UDim2.new(1 / (#self.pages_buttons / (_ - 1)), _ == 1 and 0 or 2, 0, 2);
        end;
        --
        if default then
        ...
        library:connect(button_holder.MouseButton1Click, function()
            for _,v in next, self.pages_buttons_lines do
        ...
        function page:player_list(cfg)
            ...
            -- // Drawings
            local list_holder = library:create("Square", {Parent = page_holder, Visible = true, Transparency = 1, Color = Color3.fromRGB(19,19,19), Size = UDim2.new(1,0,0,((6 * 22) + 4) + 60 + 22), Position = UDim2.new(0,0,0,0), Thickness = 1, Filled = true, ZIndex = 14}) do
            ...
            --
            local list_title = library:create("Text", {Text = "player list - 0 players", Parent = list_holder, Visible = true, Transparency = 1, Theme = "Text", Size = 13, Center = false, Outline = false, Font = Drawing.Fonts.Plex, Position = UDim2.new(0,3,0,3), ZIndex = 14});
            --
            local list_inline = library:create("Square", {Parent = list_holder, Visible = true, Transparency = 1, Color = Color3.fromRGB(13,13,13), Size = UDim2.new(1, -10, 0, (6 * 20)), Position = UDim2.new(0, 5, 0, 22), Thickness = 1, Filled = true, ZIndex = 14}) do
            ...
            local function handleoptionclick(option, button)
                button.MouseButton1Click:Connect(function()
                    ...
                    if selected_plr ~= last_plr then
                        ...
                        player_data.Text = ("id : %s\ndisplay name : %s\nname : %s\nhealth : %s/%s"):format(selected_plr.UserId, selected_plr.DisplayName ~= "" and selected_plr.DisplayName or selected_plr.Name, selected_plr.Name, "100", "100")
            ...
            local function createoptions(tbl)
                for i, option in next, tbl do
                    ...
                    optioninstances[option].text = title
                    local firstline = library:create("Square", {Transparency = 0.3,Size = UDim2.new(0,2,1,0),Position = UDim2.new(1/3,1,0,0),Parent = button,ZIndex = 19, Thickness = 1, Color = Color3.fromRGB(0,0,0), Filled = true});
                    local secondline = library:create("Square", {Transparency = 0.3,Size = UDim2.new(0,2,1,0),Position = UDim2.new(2/3,1,0,0),Parent = button,ZIndex = 19, Thickness = 1, Color = Color3.fromRGB(0,0,0), Filled = true});
            ...
            --
            left.Position = UDim2.new(0,0,0,230);
            right.Position = UDim2.new(0.5, 14,0,230);
        end;
        -- // Sections
        function page:section(cfg)
            ...
            function section:slider(cfg)
                ...
                local fade_min = cfg.fade_min or 0;
                local fade_max = cfg.fade_max or 100;
                local suffix = cfg.suffix or cfg.Suffix or "";
                local text = cfg.text or ("[value]"..suffix);
                local float = cfg.float or 1;
                local default = cfg.default and math.clamp(cfg.default, min, max) or min;
            ...
            function section:button(cfg)
                ...
                function button_tbl:button(cfg)
                    local name = cfg.name or cfg.Name or "Button";
                    local callback = cfg.callback or cfg.Callback or function() end;
                    ButtonFrame.Size = UDim2.new(1, -141, 0, 17)
                    -- Main
                    local ButtonFrame_2 = library:create("Square", {
                        ...
                        Thickness = 0,
                        Color = Color3.fromRGB(25,25,25),
                        Size = UDim2.new(1, -141, 0, 17),
                        Position = UDim2.new(0, 114, 1, -22),
                        ZIndex = 14,
                        Parent = holder
                    })
            ...
            function section:textbox(cfg)
                ...
                holder.MouseEnter:Connect(function()
                    textbox.Color = Color3.fromRGB(27,27,27)
                end)
            
                holder.MouseLeave:Connect(function()
                    textbox.Color = Color3.fromRGB(25,25,25)
                end)
                ...
                local placeholder = library:create("Text", {Text = placeholder, Transparency = 1, Visible = true, Parent = textbox, Theme = "Text", ZIndex = 14, Center = true, Position = UDim2.new(0.5, 0, 0, 1), Font = 2, Size = 13, Outline = true});
                -- functions
        ...
        function page:multisection(cfg)
            ...
            function multisection:section(cfg)
                ...
                --
                local section_content = library:create("Square", {Visible = false, Transparency = 0,Size = UDim2.new(1, -32, 1, -45),Position = UDim2.new(0, 16, 0, 50),Parent = section_holder,ZIndex = 14});
                section_content:AddListLayout(8);
...
function library:notify(info)
    ...
    --
    local line = library:create("Square", {Parent = background, Visible = true, Transparency = 1, Theme = "Accent", Size = UDim2.new(1,0,0,1), Position = UDim2.new(0,0,0,0), Thickness = 1, Filled = true, ZIndex = 11});
    --
    ...
    function ntif.idontlikemylifesucidepleasesomebodykillme(self)
        local goaway = tween.new(ntif.instances[2], TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = UDim2.new(0,-500,0,0)}):Play()
