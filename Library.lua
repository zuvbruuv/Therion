...
local RayfieldLibrary = {
	...
	Theme = {
		Default = {
			...
			SliderBackground = Color3.fromRGB(82, 105, 232),
			SliderProgress = Color3.fromRGB(82, 105, 232),
			SliderStroke = Color3.fromRGB(82, 105, 235),
			ToggleBackground = Color3.fromRGB(30, 30, 30),
			ToggleEnabled = Color3.fromRGB(82, 105, 232),
			ToggleDisabled = Color3.fromRGB(100, 100, 100),
			ToggleEnabledStroke = Color3.fromRGB(82, 105, 232),
...
if not globalSuccess then
	local errorHandling = not useStudio and game:GetObjects("rbxassetid://89663359414810")[1] or script.Parent.Error
	local coreGui = game:GetService('CoreGui')

	if gethui then
		errorHandling.Parent = gethui()
	elseif syn and syn.protect_gui then 
		syn.protect_gui(errorHandling)
		errorHandling.Parent = coreGui
	elseif not useStudio and coreGui:FindFirstChild("RobloxGui") then
		errorHandling.Parent = coreGui:FindFirstChild("RobloxGui")
	elseif not useStudio then
		errorHandling.Parent = coreGui
	end
	...
	errorHandling.Main.Okay.Interact.MouseButton1Click:Connect(function()
		errorHandling:Destroy()
	end)

	if not useStudio then
		request({
			Url = 'https://error.sirius.menu/report',
			Method = 'POST',
			Headers = {
				["Content-Type"] = "application/json"
			},
		...
		warn('Simulating Error Push')
	end

	warn('Rayfield Interface Suite has encountered an error.\nDevelopers have been notified and you can find more information below.\n\n')
	error(globalResult)
end
